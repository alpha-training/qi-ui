          ...
          <thead className="sticky top-0 bg-[#0d1117] z-10">
            <tr className="border-b border-white/10">
              {['name', 'host', 'port', 'status', 'pid', 'mem/heap', 'log', 'action'].map(h => (
                <th key={h} className="px-5 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {pageRows.map((r, i) => (
              <tr key={r.name}
                ...
                <td className="px-5 py-3.5 text-zinc-400">{r.host}</td>
                <td className="px-5 py-3.5 text-zinc-400 font-mono">{r.port}</td>
                ...
                <td className="px-5 py-3.5 text-zinc-500 font-mono">{r.pid ?? '–'}</td>
                ...
                <td className="px-5 py-3.5">
                  {r.status === 'stopped' ? (
                    <button onClick={() => startProcess(activeStack, r.name)}
                      className="px-5 py-1.5 rounded-lg border border-green-500/60 text-green-400 text-xs
                        hover:bg-green-500/10 hover:border-green-500 transition-all font-semibold min-w-[72px]">
                      Start
                    </button>
                  ) : (
                    <button onClick={() => stopProcess(activeStack, r.name)}
                      className="px-5 py-1.5 rounded-lg bg-orange-500 hover:bg-orange-400 text-white text-xs
                        transition-all font-semibold min-w-[72px] shadow-md shadow-orange-900/30">
                      Stop
                    </button>
                  )}
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-white/10 shrink-0">
          <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
            className="px-4 py-1.5 text-xs rounded-lg border border-white/10 text-zinc-400
